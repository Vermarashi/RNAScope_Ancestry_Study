#################################################################################
#################################################################################
# Title: StringTie Transcript Quantification
# Author: Rashi Verma
# Date: 2023-11-23
# Description:
#   - Quantify transcript expression from aligned RNA-seq BAM files
#   - Estimate transcript abundance using StringTie
#   - Generate transcript-level abundance files for downstream count-matrix preparation
#
# Input:
...
#
# Output:
#   - StringTie transcript abundance files
#
# Downstream:
#   - 09B_prepDE.py is used to prepare the expression count matrix
#
# Software:
#   - StringTie									
#################################################################################
...
assemble_gtf() {
    ...
    stringtie "${BAM_FILE}" -l "${PREFIX}" -p 8 -e -G "${GTF_FILE}" -o "${OUTPUT_GTF}"
